...
function ACWS_IASBY:SpawnProps()
	...
	for  i = 1, 10 do
		...
		--Workbench
		if (self.alreadySpawnedWorkbench == false and distanceToPlayer > 1 and distanceToPlayer < 50) then
...
function ACWS_IASBY:SnapToGround(object)
	if (object ~= nil) then
		local ray = Ray(object.transform.position, -object.transform.up)
		local snapToGround = Physics.Raycast(ray, Mathf.Infinity, RaycastTarget.ProjectileHit)
	
		if (snapToGround ~= nil) then
			object.transform.position = snapToGround.point
		end
	end
end
...
function ACWS_IASBY:FakeJordans()
	...
	if (soarin ~= nil) then
		if (soarin.playerCaught == false) then
