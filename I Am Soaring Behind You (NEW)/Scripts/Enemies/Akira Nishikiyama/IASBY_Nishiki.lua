...
function IASBY_Nishiki:Start()
	...
	--Notes
	self.alreadyNotedNishiki = false
...
function IASBY_Nishiki:Wander()
	...
	if (self.distanceToPlayer < 30) then
		local focusPoint = Player.actor.GetHumanoidTransformAnimated(HumanBodyBones.Chest).transform.position
		...
		if (visionRay ~= nil) then
			...
			if (not self.playerInVehicle and not self.vehicleLightsOn and not self.vehicleEngineOn and not self.playerCaught) then
				playerInSight = visionRay.collider.gameObject.GetComponentInParent(Actor)
			elseif (self.playerInVehicle and self.vehicleLightsOn or self.vehicleEngineOn and not self.playerCaught) then
				local getVehicleScript = visionRay.collider.gameObject.GetComponentInParent(Vehicle)

				if (getVehicleScript ~= nil) then
					playerInSight = getVehicleScript.driver
				end
			end
...
function IASBY_Nishiki:Chase()
	...
	--Triggers if Nishiki spots the player
	...
	if (self.distanceToPlayer <= self.closeDistance and self.alreadyAttacked == false and not self.playerInVehicle) then
		self.killSound.SetActive(true)
		self:PlayerCaught()
		self.alreadyAttacked = true
	elseif (self.distanceToPlayer <= 2.55 and self.alreadyAttacked == false and self.playerInVehicle) then
	...
	--Gives the diary idea about Nishikiyama
	if (not self.alreadyNotedNishiki) then
		local noteReader = GameObject.Find("IASBY_NoteReaderAndDiary")

		if (noteReader ~= nil) then
			local noteReaderScript = noteReader.GetComponent(IASBY_NoteReader)

			if (noteReaderScript ~= nil) then
				noteReaderScript.nishikiyamaDiary = true
				self.alreadyNotedNishiki = true
			end
		end
	end
end
...
function IASBY_Nishiki:SetWalkPoint()
	local nodeRange = Pathfinding.FindNodes(self.nishikiPos.position, 40, PathfindingNodeType.Infantry, false)

	if (#nodeRange > 0) then
		local chosenNode = nodeRange[math.random(#nodeRange)]
		local distanceToNishiki = (chosenNode.position - self.nishikiPos.position).magnitude

		if (distanceToNishiki > 1) then
			self.targetPosition = chosenNode.position
		else
			--print("chosenNode is the same node as before!")
			return function()
			end
		end
	else
		--print("nodeRange is empty!")
		return function()
		end
	end
